<?php
$steps = [
    [
        "number" => "1️⃣",
        "title" => "Browse & Select",
        "description" => "Explore our curated collection of eBooks and pick your favorites."
    ],
    [
        "number" => "2️⃣",
        "title" => "Secure Payment",
        "description" => "Pay securely using credit/debit card, GCash, or PayPal."
    ],
    [
        "number" => "3️⃣",
        "title" => "Instant Download",
        "description" => "Get your eBook instantly—read on any device, anytime."
    ]
];
?>

<section id="howitworks" class="bg-white/90 backdrop-blur-sm py-20 text-gray-900">
    <div class="mx-auto px-4 max-w-5xl text-center">
        <h3 class="mb-8 font-bold text-[#8B7E74] text-4xl header-title">
            How It Works
        </h3>

        <div class="gap-8 grid md:grid-cols-3 text-left">
            <?php foreach ($steps as $step): ?>
                <div class="bg-gray-50 hover:bg-white shadow-md p-6 rounded-xl transition-all duration-300">
                    <div class="flex justify-center items-center bg-[#8B7E74] mx-auto mb-4 rounded-full w-12 h-12 text-white text-xl">
                        <?= htmlspecialchars($step['number']) ?>
                    </div>
                    <h4 class="mb-2 font-bold text-gray-900 text-xl header-title">
                        <?= htmlspecialchars($step['title']) ?>
                    </h4>
                    <p class="text-gray-700">
                        <?= htmlspecialchars($step['description']) ?>
                    </p>
                </div>
            <?php endforeach; ?>
        </div>
    </div>
</section>